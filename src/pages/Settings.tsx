...
export default function SettingsPage() {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !user) return;
    if (!file.type.startsWith("image/")) { toast.error("אנא בחר קובץ תמונה"); return; }
    ...
    try {
      ...
      refetch();
    } catch (err: any) { toast.error(err.message ?? "שגיאה בהעלאת הלוגו"); }
  ...
  const removeLogo = async () => {
    ...
    setUploading(true);
    try {
      await supabase.from("profiles").update({ business_logo_url: null }).eq("user_id", user.id);
      ...
      <Tabs defaultValue="general" className="w-full" dir="rtl">
        <TabsList className="w-full">
          ...
          <TabsTrigger value="custom-fields" className="flex-1 gap-1.5">
            <Settings2 className="h-3.5 w-3.5" /> שדות מותאמים
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="general" className="space-y-6 mt-4">
          ...
          <Card className="border-border shadow-sm">
            <CardContent className="pt-5 space-y-4">
              ...
              <div className="space-y-1.5">
                <Label>שם העסק</Label>
                <Input placeholder="הספרייה של ישראל" value={businessName} onChange={(e) => setBusinessName(e.target.value)} />
              </div>
              ...
              {logoUrl ? (
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="ghost" onClick={removeLogo} disabled={uploading} className="text-destructive hover:text-destructive">
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </div>
                </div>
              ) : (
                <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className="flex items-center justify-center gap-2 w-full rounded-xl border-2 border-dashed border-border/60 py-6 text-sm text-muted-foreground hover:border-primary/40 hover:text-primary transition-colors">
                  {uploading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><Upload className="h-5 w-5" /> העלה לוגו</>}
                </button>
              )}
              ...
              {bookingUrl ? (
                <>
                  <div className="flex items-center gap-2 rounded-xl border border-border bg-muted/50 px-3 py-2.5">
                    <span className="flex-1 truncate text-sm font-mono text-primary">{bookingUrl}</span>
                    <button onClick={copyLink} className="flex h-7 w-7 items-center justify-center rounded-lg hover:bg-muted transition-colors text-muted-foreground hover:text-foreground">
                      {copied ? <Check className="h-3.5 w-3.5 text-success" /> : <Copy className="h-3.5 w-3.5" />}
                    </button>
                  </div>
                  <a href={bookingUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs text-primary hover:underline">
